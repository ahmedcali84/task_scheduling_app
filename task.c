...
bool Enqueue(Queue *queue , char *message)
{
    ...
    Task task = {.message = copy_msg, .id = queue->count};
...
{
    for (unsigned int i = 0; i < queue->count; ++i) {
        Log_Out(INFO, "Task ID: %d | Task: %s.\n", queue->tasks[i].id, queue->tasks[i].message);
    }
}
...
bool dump_queue(const char *file_path, const Queue *queue)
{
    ...
    size_t total_size = 0;
    for (unsigned int i = 0; i < queue->count; ++i) {
        total_size += sizeof(uint32_t) * 2; // id + msg_len
